<template>
  <q-page padding>
    <div class="row items-center q-mb-md">
      <div class="text-h4">QR Codes de Pagamento</div>
      <q-space />
      <q-btn
        color="primary"
...
async function onRequest(props) {
  ...
  try {
    ...
    const response = await axios.get('/admin/payments/qrcode', { params });
    ...
    $q.notify({
      type: 'negative',
      message: error.response?.data?.message || 'Erro ao carregar QR Codes'
    });
    qrCodes.value = [];
    pagination.value.rowsNumber = 0;
  } finally {
...
function getQrCodeUrl(qrCode) {
  ...
  return `${baseUrl}/qr/pay/${qrCode.token}`;
}
